...
| C | **统一音频引擎（主引擎）**：FFmpeg 解码 + EQ/响度/限幅/FFT/变速变调 + Opus 编码（`splayer-audio-engine`）|
| C# | 音乐库扫描（TagLibSharp，`splayer-scanner`） |
| C++ | 元数据刮削（多源并发，`splayer-scraper`） |
| Rust | 下载引擎（CLI `splayer-downloader`）、`tempo-rs`（变速变调静态库，C 引擎内置）、媒体控制/任务栏歌词（napi，Phase 3）|
| Go | Subsonic 协议层（自用桌面端可选启用） |

> **音频架构要点**：桌面端与 Web 端共用**同一条 C 引擎管线**（`splayer-audio-engine`），
...
| 扫描 | C# `splayer-scanner`（TagLibSharp + SqliteDirectWriter）| **直接复用**（子进程 + `/api/db/*`） |
| 刮削 | C++ `splayer-scraper`（多源 + 评分合并 + TagLib 写入）| **直接复用**（子进程 + `/api/db/*`） |
| 下载 | `routes/download.ts` + Rust `splayer-downloader` CLI | **直接复用** |
| 配置 | `config/store.ts` + `routes/config.ts` | **直接复用** |
| 会话/凭据 | `database/sessions.ts`（平台 cookie）| **直接复用** |
| 转码串流 | C `splayer-audio-engine` + `routes/audio.ts`（OGG/Opus）| **主引擎**：全部播放经此管线（详见 §5） |
...
│  ├─ 子进程调度：                                                   │
│  │   ├─ spawn splayer-scanner        （C#，stdout JSON lines）     │
│  │   ├─ spawn splayer-scraper        （C++，stderr 标记行）         │
│  │   ├─ spawn splayer-downloader     （Rust CLI，stdout 进度）     │
│  │   ├─ spawn splayer-audio-engine   （C，主音频引擎，见 §5）         │
...
- 原生二进制（`splayer-scanner` / `splayer-scraper` / `splayer-downloader` / `splayer-audio-engine`）：三平台构建脚本，随 app 打包分发
...
> **决策：以 C 引擎 `splayer-audio-engine` 为统一主引擎**，桌面与 Web 共用一条音频管线。
...
歌曲源（在线 URL / 本地文件 / 音乐库 trackId）
  → Flutter 直连 spawn splayer-audio-engine <source> [--eq --preamp --normalization
...
**spawn 参数表**（`splayer-audio-engine`，参数与引擎 `--help` 一致）
...
| 二进制 | 语言 | 调用方 | 协议 |
|---|---|---|---|
| `splayer-scanner` | C#（TagLibSharp）| 侧车 spawn | stdout JSON lines 进度；`POST /api/db/upsert*` 写库 |
| `splayer-scraper` | C++（libcurl + TagLib）| 侧车 spawn | stderr `[done N/Total]`；`POST /api/db/scrape/batch` |
| `splayer-downloader` | Rust（reqwest）| 侧车 spawn | stdout JSON 进度；`POST /api/db/download/*` |
| `splayer-audio-engine` | C（FFmpeg + libopus）| **主引擎**（侧车 spawn）| stdout OGG/Opus；交互模式 fd3 控制 / fd4 FFT；SIGTERM 优雅关闭 |
...
│   └── pubspec.yaml
├── audio-engine/               # C 音频引擎（纯原生库：src/ include/ CMakeLists.txt；build/ 产物 splayer-audio-engine、libfft.so）
...
| 阶段 | 内容 | 出口标准 |
|---|---|---|
| **Phase 0** 骨架 ✅ | `flutter create`（linux/windows/macos）；复制 `server/` → `sidecar/` 并打通 esbuild 打包；构建 `splayer-audio-engine`；Flutter spawn 侧车 + health 探测 + 优雅退出；HTTP/WS 打通 | **已完成（2026-08-05，见 §12.0）**：`flutter run` 起 App，侧车在线，`/api/health` 200，`/api/audio/status` 引擎可用，看门狗防孤儿 |
...
- `sidecar/audio-engine/build/splayer-audio-engine`：C 引擎（FFmpeg + Rust tempo 静态链接）按 devPath 约定落位
...
| 原生二进制 | `audio-engine/build/`（splayer-audio-engine + libfft.so）+ `tools/` 三平台构建脚本；Linux/macOS 复用 server Docker 构建链；Windows 用 vcpkg/预编译 FFmpeg 库 |
...
2. **音频主引擎：C `splayer-audio-engine`（统一转码管线）**——桌面与 Web 共用；Flutter 用 libmpv 播放 OGG/Opus 流；Rust `native/audio-engine` 不引入

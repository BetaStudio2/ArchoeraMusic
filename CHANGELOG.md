...
1、**移除汽水音源**：之前实验性接入的汽水（Soda）整块拆掉喵——Dart 侧的接口 / 登录弹窗 / 歌词 / 评论 / 红心 / 下载分支、搜索与聚合里的「汽水」来源、账号菜单里的汽水项全部清掉，Rust 下载器也去掉了 `soda` 来源变体；搜索来源切换顺手从「下拉菜单」改成紧凑分段控件（NT / KG / QM / 聚合），不再占地方喵
2、**QQ 音乐收藏页**：新增 QM 平台，分「创建的歌单 / 收藏的歌单 / 我喜欢」三栏；走官方 fcgi 纯 GET（`user_created_diss` + `get_profile_order_asset`），「我喜欢」在社区 RPC 失败时自动回退 fcg 接口，未登录显示登录引导
3、QQ 封面修复：搜索 / 歌单 / 收藏统一用 `albumMid`（缺省回退 `pmid`）拼封面，不再出现空白封面喵
4、QQ 桌面搜索重写 + **QQ 评论**接入（官方读取需登录态）
5、**聚合搜索原唱排序修复**：以前查询就是歌名时，歌手名越短反而分越高（黄龄盖过原唱尹昔眠），现在同歌名一律同分、保持各平台原生名次；同时各来源结果**轮转交错**合并，不再让某一个源整段占满列表喵
6、**主题改黑白**：主色从亮蓝改成中性灰、背景近纯黑（观感更克制），歌词默认已唱色也一并改灰
7、启动时显式刷新各平台昵称 / 头像（KG / QM），资料不再持久化，避免旧头像残留
8、启动页彩蛋：EraAudio 自研内核启动时显示「Powered by EraSync」+ 新 EraSync 字形（Stable 引擎不显示）

...
echo "== 后台执行调试脚本"
ser "setsid nohup env GV_FS=${GV_FS:-ext4} GV_ENC=${GV_ENC:-0} bash /tmp/gd.sh > /tmp/gd.log 2>&1 < /dev/null & sleep 2; echo \"GD_STARTED fs=${GV_FS:-ext4} enc=${GV_ENC:-0}\"" 60 12 | tail -3

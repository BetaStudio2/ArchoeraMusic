...
2. **内核仍不持线程**（§16.1 不变量不变）：主控只做"簿记 + 分派"，不做队列、不做后台执行；
   并发执行调度在宿主/C 壳的 worker 池（128 实例 → M 线程）。簿记计数用原子量即可，无需锁。
...
| **Async 主控** | 主控事件驱动、非阻塞（宿主事件循环 / 回调 / async-await 语法糖），只做派发 + 簿记 | 高并发任务派发 | Async 是逻辑形态≠线程；内核仍不持线程（§16.1） |
...
1. **主控 = Async（事件驱动、非阻塞）**：主控不在解码路径上忙等。职责只有三件——把任务/流分给
   ...
   - Async 是主控的**逻辑形态而非线程**：可跑在宿主单线程事件循环上，无需自建调度线程；
     **不违反"内核不持线程"**（§16.1）。
...
- 线程模型不变：`mediaengine_lib.c` 创建时启动引擎线程（转码 + 播放），事件 FIFO 与命令 FIFO、
  `compat/pthread.h`（MSVC）均原样保留——**Zig 内核本身不持线程，仅被 C 壳调用**；

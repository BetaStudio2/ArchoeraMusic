...
> **使用声明（太长不看版）**：自研代码以 **AGPL-3.0-or-later** 发布，免费开源，欢迎在
> 遵守 AGPL 条款的前提下自由使用、学习、修改与再分发——**AGPL 并不禁止商业化**：只要
> 履行其开源义务（分发或网络服务时按 AGPL 提供对应源码、以同许可证开放修改），商业目的
> 的使用同样被许可；**它禁止的只是不按条款开放的闭源商业化**。
> 本项目开发者**不提供闭源商业授权路径**：脱离 AGPL 义务的闭源/专有使用（闭源集成、
> 换壳/套壳再分发、不开放源码的商业托管服务等）未经授权不被允许，如有特殊授权需求请
> 另行联系版权方书面协商。本声明仅为开发者对授权边界的说明，法律层面以根目录
> `LICENSE`（AGPL-3.0-or-later）为准。

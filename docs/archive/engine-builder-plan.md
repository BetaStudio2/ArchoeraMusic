...
> ⚠ **更正（2026-08-16，架构调整）**：本项目已决策采用 **FFmpeg 默认主引擎 + Zig 内核渐进替换 +
> C 壳（FFI/CLI）** 路线（详见 `docs/audio-kernel-zig.md`）：`-Duse-ffmpeg` **默认开启**，FFmpeg
> 保持主解码引擎（行为零回归），Zig 内核逐格式验收后按格式接管。该决策对本规划的影响：
> - FFmpeg 仍为**默认主引擎**（动态链接 + 构建期内嵌运行库 + RUNPATH=$ORIGIN，沿用 0d5b65c），
>   本规划解决的「系统 FFmpeg major 升级破坏引擎」痛点**依然存在**；
> - 但「运行时 tcc 自编译」框架与 build.zig 构建链冲突，且内嵌运行库（0d5b65c）+ 构建时
>   `-Duse-ffmpeg` 链接方案已覆盖主要场景——**不再作为主线实施**；
> - 结论：本规划**降级为可选增强项**（仅在需要「跟随系统 FFmpeg 更新」时评估，届时优先考虑
>   「构建时链接系统 FFmpeg」而非运行时重编译），冻结为设计稿，不在近期路线图中。
...
## 8. 2026-09-06 决策：本路线已砍（随包 tcc/跟随系统 FFmpeg 取消）

> **砍掉并清理**（2026-09-06）：不再实施「随包 tcc + 自编译跟随系统 FFmpeg」，
> 也不在发行包内置 `engine-src` / `rebuild-engine.sh`（bundle 不再携带可重编源码）。
> 依据：内置（bundled）FFmpeg 为唯一后端已覆盖全部场景；自研 Zig 内核按格式渐进接管；
> 保留系统版本跟随会增加打包体积与维护面（含魔改 FFmpeg 探测/编译爆炸风险）。
> 原 §8 激活与 §9 落地设计作废删除；如需恢复再从此设计稿重评。

            ...
            child: ListView.builder(
              ...
              itemBuilder: (context, index) {
                ...
                return _SongRow(
                  ...
                  isPlaying: widget.playingId == widget.items[index].id,
                  playingNow: widget.isPlaying,
                  // 酷狗以 hash 为红心键（与 LikeController 一致）；
                  // 网易云仍用 track.id
                  liked: widget.likedIds?.contains(_likeKey(widget.items[index])) ??
                      false,
                            ...
                            Row(
                              children: [
                                ...
                                if (!prefs.hideVipTag && item.fee > 0)
                                  Padding(
                                    padding: const EdgeInsets.only(right: 6),
                                    child: _FeeBadge(fee: item.fee),
                                  ),
                                if (!prefs.hideQualityTag &&
                                    bestQuality != null)
                                  Padding(
                                    padding: const EdgeInsets.only(right: 6),
                                    child: _QualityBadge(label: bestQuality),
                                  ),
                                Flexible(
                                  child: Text(
                                    item.title,
...
/// 时长格式化（m:ss / h:mm:ss，对齐原项目 formatTime）。
...
/// 付费角标（对齐 SPlayer-Next 列表 VIP/EP 角标；fee 1=VIP，2=购买/EP）。
class _FeeBadge extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      decoration: BoxDecoration(
        border: Border.all(color: const Color(0xFFE55B5B).withValues(alpha: 0.4)),
        borderRadius: BorderRadius.circular(4),
      ),
      child: Text(
        fee == 1 ? 'VIP' : 'EP',
...
/// 音质角标（浅底小标签，标题左侧；对齐 SPlayer-Next 音质角标）。

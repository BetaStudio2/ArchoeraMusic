...
| **视口窗口化**（2026-09-20） | 只有视口 ± 自适应余量内的行参与弹簧/过渡（窗口外 `park()` 不重建求解器）；行高按需实测（视口外用估算）；重绘抑制（无变化不 notify）；失焦对齐上游（除高亮行外全模糊，半径 `min(5,1+距离)`） | 换行/每帧/字号变化的成本都从 **O(歌长) → O(视口)**；失焦仍是最大 raster 开销（可关/可半分辨率） |

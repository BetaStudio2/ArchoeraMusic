...
- **看门狗**：合成器持有被拉起会话命令的 `Child`，其进程退出即结束会话（`--no-exit-on-close`
  可关闭）；早前仅依赖 Wayland 客户端计数时，持续渲染的客户端被杀后计数可能不归零，
  故以进程退出为准。会话结束时终止可能残留的会话进程，避免孤儿。
...
**输入法生命周期（v4 后的集成修复）**：fcitx5 的启动与退出与合成器**成对**——会话脚本
在合成器 socket 就绪后才拉起 fcitx5，合成器每一轮重启都重建 fcitx5，且不用 `--replace`
（smithay 0.7 的 input-method 桥接已有旧实例时不会登记新实例，热替换会永久打断输入法）。
合成器侧另有 IME 看门狗：fcitx5 接入并抓取键盘后，连续约 3s 丢失抓取即判定断开，
主动结束会话以成对重建（`--no-ime-watch` 可关闭）；SIGTERM/SIGINT 亦优雅收尾。
